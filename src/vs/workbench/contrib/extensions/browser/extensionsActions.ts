...
export class PromptExtensionInstallFailureAction extends Action {
	...
	override async run(): Promise<void> {
		...
		if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
			const productName = isWeb ? localize('AIDE for Web', "{0} for the Web", this.productService.nameLong) : this.productService.nameLong;
	...
	override async run(): Promise<any> {
		...
		if (this.extension.deprecationInfo) {
			...
			if (this.extension.deprecationInfo.extension) {
				...
				detail = localize('deprecated with alternate settings message', "This extension is deprecated as this functionality is now built-in to AIDE.");
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload AIDE to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.deprecationInfo) {
			if (this.extension.deprecationInfo.extension) {
				...
				this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize('deprecated with alternate settings tooltip', "This extension is deprecated as this functionality is now built-in to AIDE. Configure these {0} to use this functionality.", link)) }, true);
			} else {
		...
		if (this.extension.enablementState === EnablementState.DisabledByUnification) {
			this.updateStatus({ icon: infoIcon, message: new MarkdownString(localize('extension disabled because of unification', "All AIDE functionality is now being served from the AIDE Chat extension. To temporarily opt out of this extension unification, toggle the {0} setting.", '`chat.extensionUnification.enabled`')) }, true);
			return;
		}
