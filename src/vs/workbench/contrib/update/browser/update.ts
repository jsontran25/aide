			...
			this._register(registerAction2(class SwitchQuality extends Action2 {
					...
					try {
						...
						const res = await dialogService.confirm({
							...
							detail: newQuality === 'insider' ?
								nls.localize('relaunchDetailInsiders', "Press the reload button to switch to the Insiders version of AIDE.") :
								nls.localize('relaunchDetailStable', "Press the reload button to switch to the Stable version of AIDE."),
				...
				private async selectSettingsSyncService(dialogService: IDialogService): Promise<UserDataSyncStoreType | undefined> {
					const { result } = await dialogService.prompt<UserDataSyncStoreType>({
						...
						detail: nls.localize('selectSyncService.detail', "The Insiders version of AIDE will synchronize your settings, keybindings, extensions, snippets and UI State using separate insiders settings sync service by default."),
						buttons: [
							{
