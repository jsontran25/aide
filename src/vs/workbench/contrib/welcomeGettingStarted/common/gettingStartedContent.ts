...
const AideStepTitle = localize('gettingStarted.copilotSetup.title', "Use AIDE");
const AideDescription = localize({ key: 'gettingStarted.copilotSetup.description', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "You can use [AIDE]({0}) to generate code across multiple files, fix errors, ask questions about your code, and much more using natural language.", defaultChat.documentationUrl ?? '');
const AideTermsString = localize({ key: 'gettingStarted.copilotSetup.terms', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0}, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl);
const AideAnonymousButton = Button(localize('setupAideButton.setup', "Use AIDE"), `command:workbench.action.chat.triggerSetupAnonymousWithoutDialog`);
const AideSignedOutButton = Button(localize('setupAideButton.setup', "Use AIDE"), `command:workbench.action.chat.triggerSetup`);
const AideSignedInButton = Button(localize('setupAideButton.setup', "Use AIDE"), `command:workbench.action.chat.triggerSetup`);
const AideCompleteButton = Button(localize('setupAideButton.chatWithAide', "Start Chat"), 'command:workbench.action.chat.open');

function createAideSetupStep(id: string, button: string, when: string, includeTerms: boolean): BuiltinGettingStartedStep {
	...
	return {
		...
		media: {
			type: 'svg', altText: 'AIDE multi file edits', path: 'multi-file-edits.svg'
		},
	};
}
...
export const walkthroughs: GettingStartedWalkthroughContent = [
	{
		...
		icon: setupIcon,
		when: '!isWeb',
		walkthroughPageTitle: localize('gettingStarted.setup.walkthroughPageTitle', 'Setup AIDE'),
		next: 'Beginner',
		content: {
			type: 'steps',
			steps: [
				createAideSetupStep('AideSetupAnonymous', AideAnonymousButton, 'chatAnonymous && !chatSetupInstalled', true),
				createAideSetupStep('AideSetupSignedOut', AideSignedOutButton, 'chatEntitlementSignedOut && !chatAnonymous', false),
				createAideSetupStep('AideSetupComplete', AideCompleteButton, 'chatSetupInstalled && !chatSetupDisabled && (chatAnonymous || chatPlanPro || chatPlanProPlus || chatPlanBusiness || chatPlanEnterprise || chatPlanFree)', false),
				createAideSetupStep('AideSetupSignedIn', AideSignedInButton, '!chatEntitlementSignedOut && (!chatSetupInstalled || chatSetupDisabled || chatPlanCanSignUp)', false),
				{
					id: 'pickColorTheme',
...

	{
		...
		when: 'isWeb',
		next: 'Beginner',
		walkthroughPageTitle: localize('gettingStarted.setupWeb.walkthroughPageTitle', 'Setup AIDE Web'),
		content: {
			type: 'steps',
			steps: [
				{
	...
	},
	{
		...
		description: localize('gettingStarted.setupAccessibility.description', "Learn the tools and shortcuts that make AIDE accessible. Note that some actions are not actionable from within the context of the walkthrough."),
		isFeatured: true,
		icon: setupIcon,
		when: CONTEXT_ACCESSIBILITY_MODE_ENABLED.key,
		next: 'Setup',
		walkthroughPageTitle: localize('gettingStarted.setupAccessibility.walkthroughPageTitle', 'Setup AIDE Accessibility'),
		content: {
			type: 'steps',
			steps: [
				{
				...
				},
				{
					...
					description: localize('gettingStarted.commandPaletteAccessibility.description.interpolated', "Run commands without reaching for your mouse to accomplish any task in AIDE.\n{0}", Button(localize('commandPalette', "Open Command Palette"), 'command:workbench.action.showCommands')),
			...
			steps: [
				{
					...
					description: localize('gettingStarted.settingsAndSync.description.interpolated', "Customize every aspect of AIDE and [sync](command:workbench.userDataSync.actions.turnOn) customizations across devices.\n{0}", Button(localize('tweakSettings', "Open Settings"), 'command:toSide:workbench.action.openSettings')),
